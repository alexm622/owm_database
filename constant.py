...
INSERT_TEMP = ("insert into temperature_data (location_id, temperature, feels_like, temp_min, temp_max, pressure, humidity)" 
               ...
               "%s as pressure,%s as humidity) as temp where not exists (select location_id from temperature_data where" 
               " location_id like temp.location_id and temp_min like temp.temp_min and temperature like temp.temperature "
               "and humidity like temp.humidity and pressure like temp.pressure) limit 1")
...
INSERT_WIND = ("INSERT INTO Wind_data(location_id, speed, degrees, gust)" 
                   ...
                   " location_id = temp.location_id AND speed = temp.speed AND degrees like temp.degrees" 
...
INSERT_LOC_DATA = ("INSERT INTO day_data(location_id, sunrise, sunset, timezone)" 
                   ...
                   " = temp.location_id and sunrise like temp.sunrise and"
                   " sunset like temp.sunset and timezone like temp.timezone) limit 1;")
